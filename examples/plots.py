...
def plot_power(type_II_errors, n_samples, a_list, n_trials, test, n_basis):
    """
    ...
    n_basis: list of number of basis functions
    """
    ...
    for col1, n_sample in enumerate(n_samples):
        for col2, n_b in enumerate(n_basis):
            if col2==0:
                plt.plot(a_list, type_II_errors[n_b][n_sample], colors[col1], marker='o', lw=3,
                         linestyle=linestyles[col2], label='n = {}'.format(int(n_sample)))
            else:
                plt.plot(a_list, type_II_errors[n_b][n_sample], colors[col1], marker='o', lw=3,
                         linestyle=linestyles[col2])

            error = confidence(np.asarray(type_II_errors[n_b][n_sample]), n_trials=n_trials)
            plt.fill_between(a_list, type_II_errors[n_b][n_sample] - error, type_II_errors[n_b][n_sample] + error,
